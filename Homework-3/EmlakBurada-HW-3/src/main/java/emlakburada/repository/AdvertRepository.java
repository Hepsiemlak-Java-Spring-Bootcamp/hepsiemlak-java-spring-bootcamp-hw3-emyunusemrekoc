...
public class AdvertRepository {
	...
	public List<Advert> findFavoriteAdvertsByUserId(int userId){

		//return (List<Advert>) ilanListesi.stream().filter(advert -> advert.getUserId() == userId).findAny().orElse(new Advert());
		return (List<Advert>) ilanListesi.stream().filter(advert -> advert.getUserId() == userId).collect(Collectors.toList());
